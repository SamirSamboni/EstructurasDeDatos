...
public class Discotienda
{
    ...
    // -----------------------------------------------------------------
/**
 * Generara informe de discos en la discotienda
 * @throws FileNotFoundException - Cuando no existe la ruta especifica del archivo a leer o escribir 
 */
    
    public void generarInformeDiscos1() throws FileNotFoundException
    {
    	...
    	// recorrido toral sobre discos 
    	for(int i=0;i<discos.size();i++)
    	{
    		// extraer infomrmación de cada disco
    ...
    {
    	int generoDiscoEncontrado=0;
    	int precioDiscoEncontrado=0;
    	
    	String generoEncontrado = JOptionPane.showInputDialog("Escribir género");
    	File archivo = new File ("./data/discosCostosos.txt.");
    	
    	PrintWriter pluma = new PrintWriter(archivo);
    	
    	pluma.println("Reporte Discos");
    	pluma.println("==============");
    	...
    	for (int i=0 ; i<discos.size();i++)
    	{
    		...
    		if (miDisco.darGenero().equals(generoEncontrado))
    		
    		{
    			
    			generoDiscoEncontrado++;
